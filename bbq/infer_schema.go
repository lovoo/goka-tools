...
func appendFieldSchema(metaSchema, newSchema bigquery.Schema) bigquery.Schema {
	...
	//mergeFieldsToBqSchema(metaSchema, newSchema)
	return mergeFieldsToBqSchema(metaSchema, newSchema)
}

func mergeFieldsToBqSchema(existingSchema, newSchema bigquery.Schema) bigquery.Schema {
	for _, fieldSchema := range newSchema {
		if !isIn(existingSchema, fieldSchema.Name) {
			existingSchema = append(existingSchema, fieldSchema)
		} else {
			field := getSchemaByName(existingSchema, fieldSchema.Name)
			if field != nil {
				field.Schema = mergeFieldsToBqSchema(field.Schema, fieldSchema.Schema)
			}
		}
	}
...
func isIn(s bigquery.Schema, name string) bool {
	for _, i := range s {
		if i.Name == name {
			return true
		}
...
func getSchemaByName(s bigquery.Schema, name string) *bigquery.FieldSchema {
	for _, i := range s {
		if i.Name == name {
			return i
		}
